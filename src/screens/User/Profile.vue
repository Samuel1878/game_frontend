    ...
    <div class="max-w-md mx-auto space-y-4">
        ...
        <div class="relative z-10 p-6 md:p-8 overflow-hidden">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-5">
              <div class="relative">
                <div
                  class="relative w-16 h-16 md:w-20 md:h-20 rounded-full p-0.5 bg-linear-to-b from-yellow-300 via-yellow-600 to-yellow-900 shadow-inner"
                >
                  <div
                    class="w-full h-full rounded-full bg-gray-950 overflow-hidden border-[3px] border-[#0a0f1a]"
                  >
                    <img
              ...
              <div v-if="authStore.user" class="flex flex-col justify-center">
                <h2
                  class="text-xl md:text-2xl font-black text-transparent bg-clip-text bg-linear-to-r from-white via-gray-200 to-gray-400 tracking-tight"
                >
                  {{ authStore.user?.name }}
                </h2>
                <div class="flex items-center gap-2 mt-1">
                  <div
                    class="bg-white/5 backdrop-blur-sm px-2 py-0.5 rounded text-xs text-gray-400 border border-white/5 flex items-center gap-1.5 font-mono"
                  >
                    <Phone class="w-3 h-3 text-yellow-500" />
                    <span v-if="!authStore.user?.phone">***********</span>
                    <span v-else
                      >********{{ authStore.user?.phone.slice(-3) }}</span
                    >
                  </div>
                  <!-- Subtle Status Text -->
                  <span
              ...
              >
                <div>
                  <h2
                    class="text-xl capitalize md:text-2xl font-black text-white tracking-tight group-hover/btn:text-yellow-400 transition-colors"
                  >
                    {{ t("welcome") }}
                  </h2>
                  <p
                    class="text-sm text-yellow-500/80 font-medium mt-0.5 flex items-center gap-1"
                  >
                    {{ t("login_to_your_account") }}
                  </p>
                </div>
                <div
                  class="w-10 h-10 rounded-full bg-yellow-500/10 flex items-center justify-center border border-yellow-500/20 group-hover/btn:bg-yellow-500 group-hover/btn:text-black transition-all duration-300"
                >
                  <ChevronRight class="w-5 h-5 text-yellow-500" />
                </div>
              </div>
            </div>
          </div>
          <!-- Divider -->
          <div
            class="h-px w-full bg-linear-to-r from-transparent via-white/10 to-transparent my-6"
          ></div>
          <!-- STATS GRID (Glass Cards) -->
          <div class="grid grid-cols-2 gap-3 md:gap-4 relative z-10">
            <!-- VIP Card -->
            <div
              class="bg-linear-to-br from-white/5 to-transparent border border-white/5 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 group-hover:-translate-y-1 transition-all duration-300 hover:border-yellow-500/30"
            >
              <div
                class="w-10 h-10 rounded-full bg-yellow-500/10 flex items-center justify-center border border-yellow-500/20"
              >
                <Crown class="w-5 h-5 text-yellow-500" />
              </div>
              <div>
                <p
                  class="text-[10px] md:text-xs text-gray-400 uppercase tracking-widest font-semibold mb-0.5"
                >
                  {{ t("level") }}
                </p>
                <p
                  class="text-md md:text-xl font-bold text-white drop-shadow-[0_2px_10px_rgba(255,255,255,0.2)]"
                >
            ...
            <!-- Joined Card -->
            <div
              class="bg-linear-to-br from-white/5 to-transparent border border-white/5 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 group-hover:-translate-y-1 transition-all duration-300 hover:border-sky-500/30 delay-75"
            >
              <div
                class="w-10 h-10 rounded-lg bg-sky-500/10 flex items-center justify-center border border-sky-500/20"
              >
              ...
              <div>
                ...
                <p class="text-xs md:text-md font-bold text-white">
                  {{
                    authStore.user?.created_at
                      ? moment(authStore.user?.created_at).format("DD MMM YYYY")
                      : "—"
                  }}
                </p>
              </div>
      ...
      <section class="w-full flex justify-between gap-2 items-center">
        ...
        >
          ...
          <p
            class="text-gray-300"
            :class="locale === 'mm' ? 'text-xs' : 'text-sm'"
          >
            {{ t(value.label) }}
          </p>
        </div>
