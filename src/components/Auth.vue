    ...
    <DialogContent :show-close-button="false" class="bg-gray-900 border-gray-800 text-gray-100 rounded-2xl">
      ...
      <!-- <Tabs default-value="userName"> -->
        ...
        <form class="space-y-4" @submit.prevent="submit">
          ...
          <Button
            class="w-full bg-sky-600 mt-4 text-gray-100 cursor-pointer rounded-3xl h-12 hover:bg-sky-600"
          >
            {{ isLogin ? "Login" : "Register" }}
          </Button>
        </form>
        ...
        <div class="text-center text-sm text-gray-400">
          {{ isLogin ? "Don't have an account?" : "Already have one?" }}
          <button class="ml-1 text-sky-600 cursor-pointer" @click="isLogin = !isLogin">
            {{ isLogin ? "Register" : "Login" }}
          </button>
        </div>
      <!-- </Tabs> -->
